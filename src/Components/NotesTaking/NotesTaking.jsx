import StyleNotesTaking from "./NotesTaking.module.css";
import { useState } from "react";

const NotesTaking = ({userIdClicked}) => {
      ...
      return( existingNotes.map((note, index) => (
       (userIdClicked === note.id) ?(
        <div style={{ display: "flex" , justifyContent: "space-between",marginBottom: "21px"}} key={index}>
          <div>
            <div>
            <div className={StyleNotesTaking.time}>{note.time}</div>
            <div className={StyleNotesTaking.date}>{note.date}</div>
            </div>
          </div>
         <div className={StyleNotesTaking.notes} style={{width: "50vw",contentWrap: "break-word"}}> {note.notes}</div>
         <br/><br/><br/>
        </div>
      ):(
        null
  ...
  return (
    <>
      {userIdClicked > 0 ? (
        <div className={StyleNotesTaking.NotesGroupNotes}>
          <div className={StyleNotesTaking.GroupOfNotesHeading}>
            &nbsp; &nbsp; &nbsp;<span className={StyleNotesTaking.backButton} onClick={() => window.location.reload()}><img src="Assets/BackArrowButton.png" alt="BackButton" /> &nbsp;</span>
            <div style={NotesImage}>
              {groupName[0]}
              {groupName[imageText - 1]}
            </div>
            <div className={StyleNotesTaking.NotesName}>{groupName}</div>
          </div>
          
          <div className={StyleNotesTaking.NotesContent}>
            {  reterivingMyNotes()}
          </div>
          <div className={StyleNotesTaking.NotesEnter}>
            <textarea
              type="text"
              placeholder="Enter your text here..........."
              className={StyleNotesTaking.NotesInput}
              onChange={(e) => myNotesFunction(e)}
              value={myNotes.notes}
              onKeyPress={handleKEnterKey}
            />
            <img
              src="Assets/PressEnterArrow.png"
              alt="Enter"
              className={StyleNotesTaking.NotesInputButton}
              onClick={saveMyNotes}
            />
          </div>
        </div>
